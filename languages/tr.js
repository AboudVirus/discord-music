const language = {
    ...
    msg138: "{channel}'da herhangi bir komut girmeden hızlı ve kolay bir şekilde müzik çalabilirsiniz.",
embedButton:{
   button_play: "Ekle",
   input_play: "Müziğin adını veya bağlantısını girin",
   button_autoplay: "Otomatik",
   button_volume: "Ses",
   input_volume: "{{volMin}} ile {{volMax}} arasındaki cilt numarasını girin",
   button_loop: "Döngü",
   button_back: "Geri",
   button_skip: "Atla",
   button_stop: "Durdur",
   button_filter: "Bir ses filtresi seçin",
   on: "etkin",
   off: "devre dışı bırakıldı",
   start: `
   **\`🎶\`| Yeni Çal:** {{track?.title}}
   **\`⏳\`| Süre:** {{track?.formattedDuration}}
   **\`📻\`| Kaynak:** {{track?.source}}
   **\`🎧\`| Hacim:** {{queue?.volume}}
   **\`🎛\`| Filtre:** {{queue?.filter}}
   **\`🤖\`| Otomatik Oynatma:** {{queue?.autoMode}}
   **\`🔁\`| Döngü:** {{queue?.repeatMode}}
   **\`📀\`| Şarkılar:** {{queue?.songs}}
   **\`🎙\`| Ses:** <#{{queue?.connection.channelID}}>
   **\`👤\`| Gönderen:** {{track?.user}}
   `
}
