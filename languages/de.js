   ...
   msg138: "Sie können Musik einfach und schnell abspielen, ohne einen Befehl in {channel} einzugeben.",
  embedButton:{
   button_play: "Hinzufügen",
   input_play: "Geben Sie den Namen oder Link der Musik ein",
   button_autoplay: "Auto Start",
   button_volume: "Lautstärke",
   input_volume: "{{volMax}} geben Sie die Volumennummer von {{volMin}} bis ein",
   button_loop: "Schleife",
   button_back: "Zurück",
   button_skip: "überspringen",
   button_stop: "stopp",
   button_filter: "Audiofilter auswählen",
   on: "Ermöglicht",
   off: "Nicht aktiviert",
   start: `
   **\`🎶\`| Funktioniert jetzt:** {{track?.title}}
   **\`⏳\`| Zeit:** {{track?.formattedDuration}}
   **\`📻\`| Quelle: ** {{track?.source}}
   **\`🎧\`| Volumen:** {{queue?.volume}}
   **\`🎛\`| Audiofilter :** {{queue?.filter}}
   **\`🤖\`| Auto Start:** {{queue?.autoMode}}
   **\`🔁\`| Wiederholungsmodus:** {{queue?.repeatMode}}
   **\`📀\`| Musikzähler :** {{queue?.songs}}
   **\`🎙\`| Kanal:** <#{{queue?.connection.channelID}}>
   **\`👤\`| Von:** {{track?.user}}
   `
}
   }
