const language = {
  ...
  msg138: "Vous pouvez écouter de la musique facilement et rapidement sans taper de commande dans {channel}.",
  embedButton:{
   button_play : "Ajouter",
   input_play : "Entrez le nom ou le lien de la musique",
   button_autoplay: "Automatique",
   button_volume : "Volume",
   input_volume : "{{volMax}} entrez le numéro de volume de {{volMin}} à",
   button_loop : "Boucle",
   button_back : "Retour",
   button_skip : "sauter",
   button_stop : "arrêter",
   button_filter : "Sélectionner le filtre audio",
   on: "Activé",
   off: "Pas activé",
   start: `
   **\`🎶\`| Fonctionne maintenant:** {{track?.title}}
   **\`⏳\`| Durée:** {{track?.formattedDuration}}
   **\`📻\`| Source: ** {{track?.source}}
   **\`🎧\`| Volume:** {{queue?.volume}}
   **\`🎛\`| Filtre audio:** {{queue?.filter}}
   **\`🤖\`| Lecture automatique:** {{queue?.autoMode}}
   **\`🔁\`| Mode répétition:** {{queue?.repeatMode}}
   **\`📀\`| Compteur de musique:** {{queue?.songs}}
