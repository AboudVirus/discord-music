const language = {
  ...
  msg138: "Puedes reproducir música fácil y rápidamente sin escribir un comando en {channel}.",
  embedButton:{
   button_play: "Añadir",
   input_play: "Ingrese el nombre o enlace de la música",
   button_autoplay: "Automática",
   button_volume: "Volumen",
   input_volume: "{{volMax}} ingrese el número de volumen de {{volMin}} a",
   button_loop: "Bucle",
   button_back: "Atrás",
   button_skip: "saltar",
   button_stop: "detener",
   button_filter: "Seleccionar filtro de audio",
   on: "Activado",
   off: "No disponible",
   start: `
   **\`🎶\`| Funciona ahora :** {{track?.title}}
   **\`⏳\`| Tiempo:** {{track?.formattedDuration}}
   **\`📻\`| Fuente: ** {{track?.source}}
   **\`🎧\`| Volumen:** {{queue?.volume}}
   **\`🎛\`| Filtro de audio:** {{queue?.filter}}
   **\`🤖\`| Reproducción automática:** {{queue?.autoMode}}
   **\`🔁\`| Modo de repetición:** {{queue?.repeatMode}}
   **\`📀\`| Contador de música:** {{queue?.songs}}
   **\`🎙\`| Canal:** <#{{queue?.connection.channelID}}>
   **\`👤\`| Por:** {{track?.user}}
   `
}
}
